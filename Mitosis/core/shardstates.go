...
func (s *ShardState) UpdateShardStateWithInboundChunk(inboundChunk *types.OutboundChunk) (bool, bool) { // 第一个bool代表是否是第一次收到，第二个bool代表是否header和chunk都凑齐了
	logChain.Infof("111111111InboundChunk from shard-%d block-%s has been checked successfully.", inboundChunk.Txs[0].FromShard, inboundChunk.BlockHash)

	s.mu.Lock()
	defer s.mu.Unlock()
	logChain.Infof("22222222222InboundChunk from shard-%d block-%s has been checked successfully.", inboundChunk.Txs[0].FromShard, inboundChunk.BlockHash)
	...
	logChain.Infof("333333333333InboundChunk from shard-%d block-%s has been checked successfully.", inboundChunk.Txs[0].FromShard, inboundChunk.BlockHash)
	checked := false
	header, ok := s.Headers[inboundChunk.BlockHash] // check header exists or not, if exists: verify
	logChain.Infof("4444444444444InboundChunk from shard-%d block-%s has been checked successfully.", inboundChunk.Txs[0].FromShard, inboundChunk.BlockHash)
	if ok {
		checked = s.CheckInboundChunk(header, inboundChunk)
		if checked {
			logChain.Infof("5555555InboundChunk from shard-%d block-%s has been checked successfully.", inboundChunk.Txs[0].FromShard, inboundChunk.BlockHash)
		}
	}
